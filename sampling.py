...
parser.add_argument("--ckpt_path", type=str, default='ckpts/viar.pth')
...
parser.add_argument("--iter_left", type=int, default=10)
parser.add_argument("--iter_right", type=int, default=10)
        ...
        with torch.inference_mode():
            with torch.autocast('cuda', enabled=True, dtype=torch.float16, cache_enabled=True):    # using bfloat16 can be faster
                recon_B3HW = var.autoregressive_infer_cfg(B=B, label_B=label_B, cfg=cfg, top_k=900, top_p=0.96, g_seed=seed + i, more_smooth=more_smooth, 
                                                          iter_left=args.iter_left, iter_right=args.iter_right)
