...
class VAR(nn.Module):
    def __init__(
        ...
        pre_depth=5,
        post_depth=5,
        implicit_no_grad_min_iters=0,
        implicit_no_grad_max_iters=10,
        implicit_with_grad_min_iters=1,
        implicit_with_grad_max_iters=12,
    ):
        super().__init__()
        # 0. hyperparameters
        ...
        if self.use_implicit:
            self.implicit_no_grad_min_iters = implicit_no_grad_min_iters
            self.implicit_no_grad_max_iters = implicit_no_grad_max_iters
            ...
            self.pre_blocks = nn.ModuleList([
                ...
                for block_idx in range(pre_depth)
            ])
            # implicit
            self.implicit_block = AdaLNSelfAttn(
                cond_dim=self.D, shared_aln=shared_aln,
                block_idx=depth//2, embed_dim=self.C, norm_layer=norm_layer, num_heads=num_heads, mlp_ratio=mlp_ratio,
            ...
            self.post_blocks = nn.ModuleList([
                ...
                for block_idx in range(depth-post_depth, depth)
            ])
    ...
    def autoregressive_infer_cfg(
        ...
        if self.use_implicit:
            for b in self.pre_blocks: b.attn.kv_caching(True)
        ...
        import numpy as np
        iters_list = np.linspace(iter_left, iter_right, 10, dtype=int).tolist()
        ...
        if self.use_implicit:
            for b in self.pre_blocks: b.attn.kv_caching(False)
            self.implicit_block.attn.kv_caching(False)
            for b in self.post_blocks: b.attn.kv_caching(False)
        else:
    ...
    def forward(self, label_B: torch.LongTensor, x_BLCv_wo_first_l: torch.Tensor) -> torch.Tensor:  # returns logits_BLV
        ...
        if self.use_implicit:
            # pre
            for i, b in enumerate(self.pre_blocks):
                x_BLC = b(x=x_BLC, cond_BD=cond_BD_or_gss, attn_bias=attn_bias)
            # implicit
            init_solution = x_BLC.clone()
            input_injection = x_BLC.clone()
            ...
            with nullcontext():  # we use x.detach() in place of torch.no_grad due to DDP issue
                ...
                x_BLC = self._forward_implicit_block(x_BLC=init_solution.detach(), input_injection=input_injection.detach(), 
                                                     cond_BD_or_gss=cond_BD_or_gss, attn_bias=attn_bias, num_iters=num_iters_no_grad)
                x_BLC = x_BLC.detach()  # no grad
            num_iters_with_grad = random.randint(self.implicit_with_grad_min_iters, self.implicit_with_grad_max_iters)
            x_BLC = self._forward_implicit_block(x_BLC=x_BLC, input_injection=input_injection, cond_BD_or_gss=cond_BD_or_gss, 
                                                 attn_bias=attn_bias, num_iters=num_iters_with_grad)
            # post
            for i, b in enumerate(self.post_blocks):
                x_BLC = b(x=x_BLC, cond_BD=cond_BD_or_gss, attn_bias=attn_bias)
